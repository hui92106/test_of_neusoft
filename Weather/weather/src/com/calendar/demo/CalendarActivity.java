...
import com.calendar.demo.DateWidgetDayCell.OnItemClick;
import com.example.weat.ScheduleActivity;
...
public class CalendarActivity extends Activity{
	...
	// 数据源
	ArrayList<String> Calendar_Source = new ArrayList<String>();
	...
	public static int isSunday_BgColor = 0;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Calendar c = Calendar.getInstance();
		 schedule_year = c.get(Calendar.YEAR);
		 schedule_month = c.get(Calendar.MONTH);
		 schedule_day = c.get(Calendar.DAY_OF_MONTH);
		...
		isSunday_BgColor = this.getResources().getColor(
				R.color.isSunday_BgColor);
	...
	private DateWidgetDayCell updateCalendar() {
		...
		final int thisMonth = iMonthViewCurrentMonth;
		...
		for (int i = 0; i < days.size(); i++) {
			...
			mydbhelper mdh = new mydbhelper(CalendarActivity.this);
			SQLiteDatabase sdb = mdh.getReadableDatabase();
			//要显示的列
			String[] find =new String[]{"_id","sdate","stheme"};
			String this_month = iYear+"-"+iMonth+"-"+iDay;
			//查询数据库
			Cursor cur = sdb.query("schedule", find, "sdate = ?",new String[]{this_month}, null, null, null);
			if(cur.moveToNext()){
				hasRecord = true;
			}
			mdh.close();
		    ...
		    if ((!StringUtil.isNullOrEmpty(gremessage))&&(thisMonth == iMonth)) {
		            isFestival = true;
		     } 
			...
			//是否是周末
			boolean bSunday = false;
			if ((iDayOfWeek == Calendar.SUNDAY)&&(thisMonth == iMonth))
			{
			...
			//设置背景
			dayCell.setData(iYear, iMonth, iDay, bToday, isFestival,
					iMonthViewCurrentMonth, hasRecord,bSunday);
	...
	private DateWidgetDayCell.OnItemClick mOnDayCellClick = new DateWidgetDayCell.OnItemClick() {
		public void OnClick(DateWidgetDayCell item) {
			...
			String message = "";
			String gremessage = new GregorianUtil(calSelected).getGremessage();
			
			//保存选中日期
			schedule_year = calSelected.get(Calendar.YEAR);
			schedule_month = calSelected.get(Calendar.MONTH);
	        schedule_day = calSelected.get(Calendar.DAY_OF_MONTH);
	        
	        mydbhelper mdh = new mydbhelper(CalendarActivity.this);
			SQLiteDatabase sdb = mdh.getReadableDatabase();
			//要显示的列
			String[] find =new String[]{"_id","sdate","stheme","sremindtime"};
			String this_month = schedule_year+"-"+schedule_month+"-"+schedule_day ;
			//查询数据库
			Cursor cur = sdb.query("schedule", find, "sdate = ?",new String[]{this_month}, null, null, null);
			if(cur.moveToNext()){
				String theme = cur.getString(2);
				String remind = "  （"+cur.getString(3)+"）";
				String text = theme+remind;
				arrange_text.setText(text);
				
			} 
			
			else if (!StringUtil.isNullOrEmpty(gremessage)) {
		            message = gremessage;
		            System.out.println(message);
		            arrange_text.setText(message+"节");
		     } 
			else {
				arrange_text.setText("");
			}
			...
			//设置点击监听
			item.setItemClick(new OnItemClick(){

				@Override
				public void OnClick(DateWidgetDayCell item) {
					// TODO Auto-generated method stub
					Bundle bymd = new Bundle();
					bymd.putInt("syear", schedule_year);
					bymd.putInt("smonth", schedule_month);
					bymd.putInt("sday", schedule_day);
					
					//新建日程
					Intent in = new Intent(CalendarActivity.this,ScheduleActivity.class);
					in.putExtras(bymd);
					startActivity(in);
				}
